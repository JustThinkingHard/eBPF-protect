...
* **The Kernel Hook:** An eBPF program is attached to `tracepoint/syscalls/sys_enter_write`. It extracts the true physical Inode of the executing binary (`task->mm->exe_file->f_inode->i_ino`).
    ...
    * If the binary is in the Blacklist, the LSM hook instantly returns `-EPERM` to the kernel, killing the write in-flight.
...
**Warning:** This is a live executioner. It will stop the write and truncate access to any blacklisted file. 
